...
import socket
socket.setdefaulttimeout(60)  # Установить тайм-аут по умолчанию
import threading
import json
import hashlib
import binascii
...
# Функция для завершения процесса, использующего порт
# Функция для завершения процесса, использующего порт
def ensure_port_available(port):
    for conn in psutil.net_connections(kind='inet'):
        if conn.laddr.port == port:
            try:
                ...
                print(f"[PORT IN USE] Убиваем процесс PID={conn.pid}, использующий порт :{port}")
                proc.terminate()  # Завершаем процесс
                proc.wait(timeout=10)  # Увеличиваем время ожидания до 10 секунд
                time.sleep(2)
    ...
    while True:
        ...
        if not port_in_use:
            print(f"[PORT] Порт {port} освободился.")
            break
        time.sleep(1)
# Ваши обработчики и HTTP сервер
class CommandHandler(BaseHTTPRequestHandler):
    ...
    def _shutdown_server(self):
        print("[HTTP API] Получена команда /shutdown")
        time.sleep(1)
        os._exit(0)

# Убедитесь, что порт свободен перед запуском сервера
PORT = 8080
ensure_port_available(PORT)

# Запуск HTTP API сервера
http_server = HTTPServer(('0.0.0.0', PORT), CommandHandler)
threading.Thread(target=http_server.serve_forever, daemon=True).start()

print(f"[SERVER] HTTP-сервер запущен на порту {PORT}")

# Конфигурации для Stratum Proxy
